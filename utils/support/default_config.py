from yacs.config import CfgNode as CN

C = CN()

C.DATASET = CN()

C.INPUT_PATHS = CN()
C.INPUT_PATHS.DATASET_NAME = ""
C.INPUT_PATHS.PARTITION = ""
C.INPUT_PATHS.IMAGES = ""
C.INPUT_PATHS.LABELS = ""
C.INPUT_PATHS.LABELS_NUMBERS = []
C.INPUT_PATHS.DCMS = ""
C.INPUT_PATHS.META_PATH = ""
C.INPUT_PATHS.ALPHA_CSV = ""
C.INPUT_PATHS.FHC_CSV = ""
C.INPUT_PATHS.ID_COL = ""
C.INPUT_PATHS.META_COLS = []
C.INPUT_PATHS.META_COLS_CLASSES = []

C.MODEL = CN()
C.MODEL.NAME = ""
C.MODEL.IN_CHANNELS = 0
C.MODEL.OUT_CHANNELS = 0
C.MODEL.INIT_FEATURES = 0
C.MODEL.META_FEATURES = []
C.MODEL.DEVICE = 'cpu' 
C.MODEL.ATTENTION=None
C.MODEL.FREEZE_LAYERS = 0

#for segmentation modelsOD
C.MODEL.BATCH_NORM_DECODER=True
C.MODEL.ENCODER_NAME = "resnet34"
C.MODEL.ENCODER_WEIGHTS = "imagenet"
C.MODEL.DYNAMIC_IMG_SIZE = True
C.MODEL.CHANNEL_TYPE = "none"
C.MODEL.DECODER_CHANNELS = []
C.MODEL.IN_CHANNELS = 1


C.DATASET = CN()
C.DATASET.COMBINE_REVIEWERS=False
C.DATASET.IMAGE_EXT=".png"
C.DATASET.ANNOTATION_TYPE= "LANDMARKS" 
C.DATASET.NUM_LANDMARKS = 5
C.DATASET.PIXEL_SIZE = [1,1]
C.DATASET.CACHED_IMAGE_SIZE= [512,256]
C.DATASET.SIGMA = 0 #if sigma is NONE no guassian applied
C.DATASET.FLIP_AXIS = False


C.DATASET.AUGMENTATION = CN()
C.DATASET.AUGMENTATION.APPLY = True
C.DATASET.AUGMENTATION.ROTATION_FACTOR = 0
C.DATASET.AUGMENTATION.INTENSITY_FACTOR = 0.0
C.DATASET.AUGMENTATION.SF = 0.0
C.DATASET.AUGMENTATION.TRANSLATION_X = 0.0
C.DATASET.AUGMENTATION.TRANSLATION_Y = 0.0
C.DATASET.AUGMENTATION.ELASTIC_STRENGTH = 0
C.DATASET.AUGMENTATION.ELASTIC_SMOOTHNESS = 0
C.DATASET.AUGMENTATION.SPECKLE_NOISE = 0
C.DATASET.AUGMENTATION.COARSE_SALTANDPEPPER = 0.0
C.DATASET.AUGMENTATION.SAVE= True
C.DATASET.AUGMENTATION.SOME_OF = 0
C.DATASET.PARTITION_SIZE_TEST = None

C.OUTPUT_PATH = CN()
C.OUTPUT_PATH = "/output"

C.TRAIN = CN()
C.TRAIN.BATCH_SIZE = 8
C.TRAIN.GRAD_ACCUMULATION_STEPS = 1
C.TRAIN.GRAD_ACCUMULATION_STEPS_REDUCE = 50 ## num of epochs to reduce grad accum steps by half 
C.TRAIN.GRAD_ACCUMULATION_STEPS_MIN = 4 ## 
C.TRAIN.LR = 0.001
C.TRAIN.EPOCHS = 10
C.TRAIN.LOSS = ''
C.TRAIN.L2_REG = True
C.TRAIN.GAMMA = 0.0
C.TRAIN.COMPILE = False
C.TRAIN.COMBINE_GRAF_FHC=False
C.TRAIN.SAVE_VAL_DCM=False
C.TRAIN.MOMENTUM = 0.9
C.TRAIN.USE_AMP = True
C.TRAIN.DELAY_GUMBEL_LOSS = 100 ##number of iterations (each batch iter) of just NLL before you switch to gumbel
C.TRAIN.TAU=1.0 ## each iter will decay tau by this amount 
C.TRAIN.TAU_DECAY=0.01 ## each iter will decay tau by this amount 
C.TRAIN.EARLY_STOPPING = True ## make none if you dont want it - defaults are in training
C.TRAIN.PLOT_MULTIMODAL_CHANNELS = False
C.TRAIN.PLOT_MULTIMODAL_CHANNELS_MAX_SAMPLES = 10


C.TEST = CN()
C.TEST.NETWORK = ''
C.TEST.COMPARISON_METRICS = []
C.TEST.SDR_THRESHOLD = []
C.TEST.SDR_UNITS = ''
C.TEST.SAVE_TXT = False
C.TEST.SAVE_HEATMAPS_ALONE = False
C.TEST.SAVE_HEATMAPS_LANDMARKS_IMG = False
C.TEST.SAVE_HEATMAPS_ASDCM = False
C.TEST.SAVE_IMG_LANG_PREDANDTRUE = False
C.TEST.PLOT = False
C.TEST.SHOW_ALL_LANDMARKS = True
C.TEST.SAVE_HEATMAPS_NP = False
C.TEST.PLOT_MULTIMODAL_CHANNELS = False
C.TEST.BATCH_SIZE = 1
C.TEST.NUM_WORKERS = 0
C.TEST.PIN_MEMORY = False
C.TEST.PERSISTENT_WORKERS = False
C.TEST.PREFETCH_FACTOR = 2
C.TEST.TEST_TIME_AUG = False
C.TEST.TEST_TIME_AUG_MODE = "disabled"
C.TEST.TEST_TIME_AUG_NUM = 5
C.TEST.TTA_SELECTION_METRIC = "ERE"
C.TEST.WANDB_NUM_FAILURE_EXAMPLES = 10
C.TEST.EVAL_DATASETS = []

def get_cfg_defaults():
    """Get a yacs CfgNode object with default values above."""
    return C.clone()
